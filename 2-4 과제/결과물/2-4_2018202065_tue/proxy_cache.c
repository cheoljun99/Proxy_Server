...
static void handler(int sig)
{
	...
	{
		printf("================No Responese=========================\n");
		//핸들러에서 시그알람일 때 만약 서브프로세스를 종료한다면 사용//bye(0,1,10);
		...
		if (pid == 0)
		{
			...
			if (strcmp(method, "GET") == 0)// Get 메소드의 request일때 URL을 통해 cache생성
			{
				...
				*/
				...
				int check = sub_server_processing_helper(hash_passing_url);// 추출한 URL을 과제 1-2의 main함수였지만 현재는 sub_server_processing_helper 함수에게 인자로 넘겨줌
				if (check > 0)
				{
					//Hit일 경우
					//클라이언트에게 hit을 전달함
				...
				else if (check == 0)
				{
					...
					else
					{
						...
						*/
						...
						{
							//read를 실패함을 알려주는 출력문 //printf("while end point read_num is %d\n", read_num);
							close(server_fd);
						}
						close(server_fd);
					}
