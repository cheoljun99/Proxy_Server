		...
		if (pid == 0)
		{
			...
			if (strcmp(method, "GET") == 0)// Get 메소드의 request일때 URL을 통해 cache생성
			{
				...
				*/
				...
				int check = sub_server_processing_helper(url);// 추출한 URL을 과제 1-2의 main함수였지만 현재는 sub_server_processing_helper 함수에게 인자로 넘겨줌
				if (check > 0)
				{
				...
				else if (check == 0)
				{
					...
					sprintf(response_header,
						...
						"Content - length:%lu\r\n"
						"content-type:text/html\r\n\r\n", strlen(response_message));
					if (sub_process_webserver_communication(tok, buf) < 0)//웹서버와 통신을 하는 함수!!!!
					{
						//sub process fails webserver comunication in read or connect function and get ip addr
					}
