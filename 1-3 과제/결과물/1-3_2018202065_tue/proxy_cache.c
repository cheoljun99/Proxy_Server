...
int sub_process(void)
{
	///////////////////////////////////////////////////////////////////////
	// sub_process 함수													 //	
	// ==================================================================//
	// 																	 //
	// 목적: 해쉬화된 URL을 Cache에 저장하는것이                         //
	// 목표다.                        		                             //
	// 																	 //
	// 																	 //
	///////////////////////////////////////////////////////////////////////
...
int main(void)
{
	...
	// 이용하여 본 과제의 목적인 multiple - processing을 구현한다.       //
	// 																	 //
	// 																	 //
	///////////////////////////////////////////////////////////////////////

	pid_t childPid;//fork함수의 리턴값을 저장할 변수
	int status;// status를 저장할 변수
	char command[100];//입력받을 커맨드를 저장할 변수
	time_t tstart, tend;// 프로세스 시작과 끝을 저장할 변수
	time(&tstart);//메인 프로세스의 시작을 저장
	int subcount=0;//서브프로세스 생성개수를 저장할 변수

	char dire[100]; // 사용하고자 하는 디렉토리 위치를 저장할 변수

	getHomeDir(dire);//홈디렉토리를 dire에 저장
	...
	while (1)
	{
		...
		if (!strcmp(command, "quit"))//quit 코멘트가 들어가면 실행 종료.
		{
		...
		if (!strcmp(command, "connect"))//connect 코멘트가 들어가면 서브프로세스 생성 및 실행.
		{
			subcount++;//서브프로세스 생성 개수를 기록하는 변수
			childPid = fork(); // child프로세스 생성
			if (childPid > 0) {  // 부모 프로세스
				pid_t waitPid;
				waitPid = wait(&status);// child프로세스가 끝날때까지 기다린다.
